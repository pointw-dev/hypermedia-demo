...
from hypermea.core.utils import (make_error_response, unauthorized_message,
                                 get_resource_id, get_my_base_url, get_db)
...
def _do_register_event(event_id, account):
    ...
    account_url = f'http://accounts-service/accounts/{username}'
    response = requests.get(account_url, headers={'Authorization': f'Basic cm9vdDpwYXNzd29yZA=='})  # NOTE: this would ordinarily be configured for this service to be an authorized client of the accounts-service
    account = None
    ...
    return _register({'_event_ref': event_id, '_account_ref': account['_id']},
                     username if not username == my_username else None)


def _register(registration, username=None):
    registrations = get_db()['registrations']
    event_id = registration['_event_ref']
    ...
    attendee_count = registrations.count_documents({'_event_ref': event_id})
    ...
    registration_url = f'http://registrations-service/registrations/event/{event_id}'
    response = requests.post(registration_url, headers=headers, data=json.dumps(registration, default=str))
