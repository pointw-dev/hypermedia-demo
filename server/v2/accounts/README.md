...
The audience of this README is the **client developer**, i.e. someone who will **use** `accounts-service`.

If you are an **API developer** who will make changes to `accounts-service` itself, please see [`src/accounts-service/README.md`](./src/accounts-service/README.md).
...
`accounts-service` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `accounts-service`
...
* `accounts-service` follows a field naming convention so you know when such a field is "embeddable"
