...
from hypermea.core.utils import get_db, make_error_response
...
def _add_links_to_event(event):
    ...
    if event.get('_venue_ref'):
        event['_links']['parent'] = {
            'href': f'{base_url}/venues/{event["_venue_ref"]}',
            'title': 'venues'
        }
        event['_links']['collection'] = {
            'href': f'{base_url}/venues/{event["_venue_ref"]}/events',
            'title': 'venue_events'
        }
    else:
        event['_links']['parent'] = {
    ...
    event['_links']['registrations'] = {
        'href': f'{base_url}/events/{event_id}/registrations',
        'title': 'registrations'
    }
