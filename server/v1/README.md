...
The audience of this README is the **client developer**, i.e. someone who will **use** `event-buddy`.

If you are an **service developer** who will make changes to `event-buddy` itself, please see [`src/event-buddy/README.md`](./src/event-buddy/README.md).
...
`event-buddy` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `event-buddy`
...
`event-buddy` is built with [Eve](https://docs.python-eve.org/en/stable/) and enhanced by [hypermea](https://github.com/pointw-dev/hypermea).  What follows is a list of some of the features provided by Eve.  You can learn more from its [feature documentation](https://docs.python-eve.org/en/stable/features.html).
...
* `event-buddy` follows a field naming convention so you know when such a field is "embeddable"
