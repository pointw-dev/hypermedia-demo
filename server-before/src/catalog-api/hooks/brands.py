...
def add_hooks(app):
    ...
    app.on_fetched_item_regions_brands += _add_links_to_brand
    app.on_fetched_resource_regions_brands += _add_links_to_brands_collection
    app.on_post_POST_regions_brands += _post_brands
...
def _add_links_to_brand(brand):
    ...
    if brand.get('_region_ref'):
        brand['_links']['parent'] = {
            'href': f'{base_url}/regions/{brand["_region_ref"]}',
            'title': 'regions'
        }
        brand['_links']['collection'] = {
            'href': f'{base_url}/regions/{brand["_region_ref"]}/brands',
            'title': 'region_brands'
        }
    else:
        brand['_links']['parent'] = {
