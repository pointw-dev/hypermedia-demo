...
The audience of this README is the **client developer**, i.e. someone who will **use** `catalog-api`.

If you are an **API developer** who will make changes to `catalog-api` itself, please see [`src/catalog-api/README.md`](./src/catalog-api/README.md).
...
`catalog-api` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `catalog-api`
...
* `catalog-api` follows a field naming convention so you know when such a field is "embeddable"
