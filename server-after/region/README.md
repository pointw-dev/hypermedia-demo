...
The audience of this README is the **client developer**, i.e. someone who will **use** `region-api`.

If you are an **API developer** who will make changes to `region-api` itself, please see [`src/region-api/README.md`](./src/region-api/README.md).
...
`region-api` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `region-api`
...
* `region-api` follows a field naming convention so you know when such a field is "embeddable"
