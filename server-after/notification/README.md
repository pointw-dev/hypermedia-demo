...
The audience of this README is the **client developer**, i.e. someone who will **use** `notification-api`.

If you are an **API developer** who will make changes to `notification-api` itself, please see [`src/notification-api/README.md`](./src/notification-api/README.md).
...
`notification-api` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `notification-api`
...
* `notification-api` follows a field naming convention so you know when such a field is "embeddable"
